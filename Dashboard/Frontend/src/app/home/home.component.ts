...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  private get batchApiConfig() {
    return {
      requests: [
        ...
        { id: 'jobApplications', method: 'GET', url: '/api/job-applications?$count=true' },
        { id: 'faqList', method: 'GET', url: '/odata/Faqs?$count=true' },
        { id: 'commonFormData', method: 'GET', url: '/odata/CommonForms?$count=true' },
        { id: 'contactUsFormData', method: 'GET', url: '/odata/ContactUsForms?$count=true' }
      ]
    };
  }

  getCountValue(): void {
    this.commonService.post('$batch', this.batchApiConfig)
      .pipe(
  ...
  private processBatchResponse(responses: BatchResponse[]): void {
    ...
    this.countArray = [
      this.createCountItem('Services', counts['services'], 'database', ['/service-list']),
      this.createCountItem('Job Posts', counts['jobPosts'], 'business-card', ['/job-list']),
      this.createCountItem('Job Applications', counts['jobApplications'], 'employee', ['/job-applications']),
      this.createCountItem('FAQs', counts['faqList'], 'question-mark', ['/faq-list']),
      this.createCountItem('Common Forms', counts['commonFormData'], 'form', ['/common-form']),
      this.createCountItem('Contact Us', counts['contactUsFormData'], 'email', ['/contact-us-form'])
    ];
  }
  ...
  private initializeWithEmptyData(): void {
    this.countArray = [
      ...
      this.createCountItem('Job Applications', 0, 'employee', ['job-applications']),
